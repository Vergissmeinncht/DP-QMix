...
def calculate_n_step_td_target(mixer, target_mixer, target_max_qvals, batch, rewards, terminated, mask, gamma,
    ...
    with th.no_grad():
        ...
        if mixer == "qgattenmix" or mixer == "qghypermix":
            ...
            target_max_qvals = target_mixer(target_max_qvals, batch["state"], batch["obs"], mac_out,
                                            batch["death_mask"])
        else:
...
class NQLearner:
    ...
    def train_predict(self, batch: EpisodeBatch, t_env: int):
        # 提取需要的数据
        states = batch["obs"][:, :-1]  # (B, T-1, N, obs_dim)
        actions = batch["actions_onehot"][:, :-1]  # (B, T-1, N, action_dim)
        masks = batch["death_mask"][:, :-1]  # (B, T-1, N)

        # 获取下一状态和下一死亡掩码作为标签
        next_states = batch["obs"][:, 1:].detach()  # (B, T-1, N, obs_dim)
        next_masks = batch["death_mask"][:, 1:].detach()  # (B, T-1, N)

        loss = self.mixer.death_prediction_model.update(states, actions, masks, next_states, next_masks)
        for loss_name, loss_value in loss.items():
            self.logger.log_stat(f"death_pred_{loss_name}", loss_value, t_env)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        with th.no_grad():
            ...
            if self.args.mixer.find("qmix") != -1 and self.enable_parallel_computing:
                targets = self.pool.apply_async(
                    ...
                    (self.args.mixer, self.target_mixer, target_max_qvals, batch, rewards, terminated, mask,
                     self.args.gamma,
                     self.args.td_lambda, True, self.args.thread_num, False, None, mac_out)
                )
            else:
                targets = calculate_n_step_td_target(
                    self.args.mixer, self.target_mixer, target_max_qvals, batch, rewards, terminated, mask,
                    self.args.gamma,
                    self.args.td_lambda, False, 4, False, None, mac_out
                )
        ...
        elif self.args.mixer == "dqmix":

            chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1], batch["obs"][:, :-1],
                                             mac_out[:, :-1], deadth_mask[:, :-1])
        else:
