...
def run(_run, _config, _log):
    ...
    if args.name in testing_algorithms:
        ...
        elif args.name in ["for_hpn_qgattenmix"]:
            logdir = os.path.join(logdir,
                                  "head_n={}-mixer={}-hpn_hyperdim={}-acti={}-tdlambda={}-beta1_{}_beta2_{}".format(
                                      args.hpn_head_num,
                                      args.mixer,
                                      args.hpn_hyper_dim,
                                      args.hpn_hyper_activation,
...
def run_sequential(args, logger):
    ...
    scheme = {
        ...
        "formation": {"vshape": (args.n_agents * 4,), "dtype": th.float32},
        "max_idx": {"vshape": (args.n_agents,), "dtype": th.int64},
        "min_idx": {"vshape": (args.n_agents,), "dtype": th.int64},
        "visit": {"vshape": (1,), "dtype": th.int64},
        "death_mask": {
            "vshape": (args.n_agents,),  # 掩码形状为 (n_agents,)
            "dtype": th.int64,
        }
    }
    groups = {
    ...
    while runner.t_env <= args.t_max:
        ...
        if buffer.can_sample(args.batch_size):
            ...
            if args.enable_death_prediction:
